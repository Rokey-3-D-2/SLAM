...
# 상수 정의
# ========================
MODEL_PATH = '/home/choin/rokey_ws/model/best.pt'         # YOLO 모델 경로 경로
    ...
    def timer_callback(self):
        ...
        try:
            ...
            try:
                ...
                self.get_logger().info(f"[TF] image → map ({point_map.point.x:.2f}, {point_map.point.y:.2f}, {point_map.point.z:.2f})")
        ...
        while rclpy.ok():
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    # RGB 이미지 위 시각화
                    cv2.rectangle(rgb, (x1, y1), (x2, y2), (0, 255, 0), 2)  # object detection
                    cv2.circle(rgb, (u, v), 4, (0, 0, 255), -1)             # depth
                    cv2.putText(rgb, f"{label}: {conf}, {distance_m:.2f}m", (x1, y1 - 10),
